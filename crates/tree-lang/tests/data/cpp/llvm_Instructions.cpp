...
static cl::opt DisableI2pP2iOpt(
...
AllocaInst::getAllocationSizeInBits(const DataLayout &DL) const {
 std::optional Size = getAllocationSize(DL);
 if (!Size)
 return std::nullopt;
...
const char *SelectInst::areInvalidOperands(Value *Op0, Value *Op1, Value *Op2) {
 ...
 if (VectorType *VT = dyn_cast (Op0->getType())) {
 // Vector select.
 if (VT->getElementType() != Type::getInt1Ty(Op0->getContext()))
 return "vector select condition element type must be i1";
 VectorType *ET = dyn_cast (Op1->getType());
 if (!ET)
 ...
 return "select condition must be i1 or ";
 }
 return nullptr;
}
...
void PHINode::removeIncomingValueIf(function_ref Predicate,
...
CallBase *CallBase::Create(CallBase *CB, ArrayRef Bundles,
 InsertPosition InsertPt) {
 switch (CB->getOpcode()) {
 case Instruction::Call:
 return CallInst::Create(cast (CB), Bundles, InsertPt);
 case Instruction::Invoke:
 return InvokeInst::Create(cast (CB), Bundles, InsertPt);
 case Instruction::CallBr:
 return CallBrInst::Create(cast (CB), Bundles, InsertPt);
 default:
...
unsigned CallBase::getNumSubclassExtraOperandsDynamic() const {
 ...
 return cast (this)->getNumIndirectDests() + 1;
}
...
Intrinsic::ID CallBase::getIntrinsicID() const {
 if (auto *F = dyn_cast_or_null (getCalledOperand()))
...
std::optional CallBase::getRange() const {
...
CallBase::populateBundleOperandInfos(ArrayRef Bundles,
...
void CallInst::init(FunctionType *FTy, Value *Func, ArrayRef Args,
 ArrayRef Bundles, const Twine &NameStr) {
 this->FTy = FTy;
...
CallInst *CallInst::Create(CallInst *CI, ArrayRef OpB,
 InsertPosition InsertPt) {
 std::vector Args(CI->arg_begin(), CI->arg_end());
...
void InvokeInst::init(FunctionType *FTy, Value *Fn, BasicBlock *IfNormal,
 BasicBlock *IfException, ArrayRef Args,
 ArrayRef Bundles,
 const Twine &NameStr) {
 this->FTy = FTy;
...
InvokeInst *InvokeInst::Create(InvokeInst *II, ArrayRef OpB,
 InsertPosition InsertPt) {
 std::vector Args(II->arg_begin(), II->arg_end());
...
LandingPadInst *InvokeInst::getLandingPadInst() const {
 return cast (getUnwindDest()->getFirstNonPHIIt());
}
...
CallBrInst *CallBrInst::Create(CallBrInst *CBI, ArrayRef OpB,
 InsertPosition InsertPt) {
 std::vector Args(CBI->arg_begin(), CBI->arg_end());
...
//===----------------------------------------------------------------------===//
void FuncletPadInst::init(Value *ParentPad, ArrayRef Args,
 const Twine &NameStr) {
...
FuncletPadInst::FuncletPadInst(Instruction::FuncletPadOps Op, Value *ParentPad,
 ArrayRef Args, AllocInfo AllocInfo,
 const Twine &NameStr,
 InsertPosition InsertBefore)
...
bool AllocaInst::isArrayAllocation() const {
 if (ConstantInt *CI = dyn_cast (getOperand(0)))
 return !CI->isOne();
 return true;
}
...
bool AllocaInst::isStaticAlloca() const {
 ...
 if (!isa (getArraySize())) return false;
...
Type *GetElementPtrInst::getTypeAtIndex(Type *Ty, Value *Idx) {
 ...
 return nullptr;
 if (auto *Array = dyn_cast (Ty))
 return Array->getElementType();
 if (auto *Vector = dyn_cast (Ty))
...
template 
static Type *getIndexedTypeInternal(Type *Ty, ArrayRef IdxList) {
 if (IdxList.empty())
 return Ty;
...
Type *GetElementPtrInst::getIndexedType(Type *Ty, ArrayRef IdxList) {
...
bool GetElementPtrInst::hasAllZeroIndices() const {
 ...
 if (ConstantInt *CI = dyn_cast (getOperand(i))) {
...
void GetElementPtrInst::setIsInBounds(bool B) {
 GEPNoWrapFlags NW = cast (this)->getNoWrapFlags();
 if (B)
...
GEPNoWrapFlags GetElementPtrInst::getNoWrapFlags() const {
 return cast (this)->getNoWrapFlags();
}
...
bool GetElementPtrInst::hasNoUnsignedSignedWrap() const {
 return cast (this)->hasNoUnsignedSignedWrap();
}
...
bool GetElementPtrInst::accumulateConstantOffset(const DataLayout &DL,
 ...
 return cast (this)->accumulateConstantOffset(DL, Offset);
}
...
bool GetElementPtrInst::collectOffset(
 ...
 // Delegate to the generic GEPOperator implementation.
 return cast (this)->collectOffset(DL, BitWidth, VariableOffsets,
 ConstantOffset);
}
...
ShuffleVectorInst::ShuffleVectorInst(Value *V1, ArrayRef Mask,
 const Twine &Name,
 InsertPosition InsertBefore)
 : ShuffleVectorInst(V1, createPlaceholderForShuffleVector(V1), Mask, Name,
 InsertBefore) {}
...
ShuffleVectorInst::ShuffleVectorInst(Value *V1, Value *V2, Value *Mask,
 ...
 InsertPosition InsertBefore)
 : Instruction(
 VectorType::get(cast (V1->getType())->getElementType(),
 cast (Mask->getType())->getElementCount()),
 ...
 Op<1>() = V2;
 SmallVector MaskArr;
 getShuffleMask(cast (Mask), MaskArr);
 setShuffleMask(MaskArr);
 setName(Name);
}

ShuffleVectorInst::ShuffleVectorInst(Value *V1, Value *V2, ArrayRef Mask,
 const Twine &Name,
 InsertPosition InsertBefore)
 : Instruction(
 VectorType::get(cast (V1->getType())->getElementType(),
 Mask.size(), isa (V1->getType())),
 ShuffleVector, AllocMarker, InsertBefore) {
 assert(isValidOperands(V1, V2, Mask) &&
 "Invalid shuffle vector instruction operands!");
 Op<0>() = V1;
...
void ShuffleVectorInst::commute() {
 ...
 SmallVector NewMask(NumMaskElts);
...
bool ShuffleVectorInst::isValidOperands(const Value *V1, const Value *V2,
 ...
 if (!isa (V1->getType()) || V1->getType() != V2->getType())
 return false;

 // Make sure the mask elements make sense.
 int V1Size =
 cast (V1->getType())->getElementCount().getKnownMinValue();
 for (int Elem : Mask)
...
bool ShuffleVectorInst::isValidOperands(const Value *V1, const Value *V2,
 ...
 // input vectors
 auto *MaskTy = dyn_cast (Mask->getType());
 if (!MaskTy || !MaskTy->getElementType()->isIntegerTy(32) ||
 isa (MaskTy) != isa (V1->getType()))
 return false;

 // Check to see if Mask is valid.
 if (isa (Mask) || isa (Mask))
 return true;
 ...
 unsigned V1Size = cast (V1->getType())->getNumElements();

 if (const auto *CI = dyn_cast (Mask))
 return !CI->uge(V1Size * 2);

 if (const auto *MV = dyn_cast (Mask)) {
 ...
 if (const auto *CDS = dyn_cast (Mask)) {
 for (unsigned i = 0, e = cast (MaskTy)->getNumElements();
 i != e; ++i)
...
void ShuffleVectorInst::getShuffleMask(const Constant *Mask,
 ...
 ElementCount EC = cast (Mask->getType())->getElementCount();

 if (isa (Mask) || isa (Mask)) {
 int MaskVal = isa (Mask) ? -1 : 0;
 ...
 Result.push_back(isa (C) ? -1 :
 cast (C)->getZExtValue());
 }
}

void ShuffleVectorInst::setShuffleMask(ArrayRef Mask) {
 ShuffleMask.assign(Mask.begin(), Mask.end());
 ShuffleMaskForBitcode = convertShuffleMaskForBitcode(Mask, getType());
}

Constant *ShuffleVectorInst::convertShuffleMaskForBitcode(ArrayRef Mask,
 Type *ResultTy) {
...
static bool isSingleSourceMaskImpl(ArrayRef Mask, int NumOpElts) {
...
bool ShuffleVectorInst::isSingleSourceMask(ArrayRef Mask, int NumSrcElts) {
...
static bool isIdentityMaskImpl(ArrayRef Mask, int NumOpElts) {
...
bool ShuffleVectorInst::isIdentityMask(ArrayRef Mask, int NumSrcElts) {
 if (Mask.size() != static_cast (NumSrcElts))
 return false;
 // We don't have vector operand size information, so assume operands are the
 // same size as the mask.
...
bool ShuffleVectorInst::isReverseMask(ArrayRef Mask, int NumSrcElts) {
 if (Mask.size() != static_cast (NumSrcElts))
 return false;
...
bool ShuffleVectorInst::isZeroEltSplatMask(ArrayRef Mask, int NumSrcElts) {
 if (Mask.size() != static_cast (NumSrcElts))
 return false;
 if (!isSingleSourceMask(Mask, NumSrcElts))
 return false;
...
bool ShuffleVectorInst::isSelectMask(ArrayRef Mask, int NumSrcElts) {
 if (Mask.size() != static_cast (NumSrcElts))
 return false;
...
bool ShuffleVectorInst::isTransposeMask(ArrayRef Mask, int NumSrcElts) {
 // Example masks that will return true:
 // v1 = 
 // v2 = 
 // trn1 = shufflevector v1, v2 <0, 4, 2, 6> = 
 // trn2 = shufflevector v1, v2 <1, 5, 3, 7> = 
...
bool ShuffleVectorInst::isSpliceMask(ArrayRef Mask, int NumSrcElts,
 int &Index) {
 if (Mask.size() != static_cast (NumSrcElts))
 return false;
...
bool ShuffleVectorInst::isExtractSubvectorMask(ArrayRef Mask,
...
bool ShuffleVectorInst::isInsertSubvectorMask(ArrayRef Mask,
 ...
 ArrayRef Sub1Mask = Mask.slice(Src1Lo, NumSub1Elts);
 ...
 ArrayRef Sub0Mask = Mask.slice(Src0Lo, NumSub0Elts);
...
static bool isReplicationMaskWithParams(ArrayRef Mask,
 int ReplicationFactor, int VF) {
 assert(Mask.size() == (unsigned)ReplicationFactor * VF &&
 "Unexpected mask size.");

 for (int CurrElt : seq(VF)) {
 ArrayRef CurrSubMask = Mask.take_front(ReplicationFactor);
...
bool ShuffleVectorInst::isReplicationMask(ArrayRef Mask,
 ...
 reverse(seq_inclusive (1, Mask.size()))) {
...
bool ShuffleVectorInst::isReplicationMask(int &ReplicationFactor,
 ...
 VF = cast (Op<0>()->getType())->getNumElements();
...
bool ShuffleVectorInst::isOneUseSingleSourceMask(ArrayRef Mask, int VF) {
 if (VF <= 0 || Mask.size() < static_cast (VF) ||
 Mask.size() % VF != 0)
 return false;
 for (unsigned K = 0, Sz = Mask.size(); K < Sz; K += VF) {
 ArrayRef SubMask = Mask.slice(K, VF);
...
bool ShuffleVectorInst::isInterleave(unsigned Factor) {
 FixedVectorType *OpTy = dyn_cast (getOperand(0)->getType());
...
bool ShuffleVectorInst::isInterleaveMask(
 ArrayRef Mask, unsigned Factor, unsigned NumInputElts,
 SmallVectorImpl &StartIndexes) {
...
/// \p Factor like:
/// 
bool ShuffleVectorInst::isDeInterleaveMaskOfFactor(ArrayRef Mask,
 unsigned Factor,
 unsigned &Index) {
 ...
 // elements.
 for (; I < Mask.size(); I++)
 if (Mask[I] >= 0 && static_cast (Mask[I]) != Idx + I * Factor)
 break;
...
static int matchShuffleAsBitRotate(ArrayRef Mask, int NumSubElts) {
...
bool ShuffleVectorInst::isBitRotateMask(
 ArrayRef Mask, unsigned EltSizeInBits, unsigned MinSubElts,
...
void InsertValueInst::init(Value *Agg, Value *Val, ArrayRef Idxs,
 const Twine &Name) {
...
Type *ExtractValueInst::getIndexedType(Type *Agg,
 ...
 if (ArrayType *AT = dyn_cast (Agg)) {
 if (Index >= AT->getNumElements())
 return nullptr;
 Agg = AT->getElementType();
 } else if (StructType *ST = dyn_cast (Agg)) {
...
float FPMathOperator::getFPAccuracy() const {
 const MDNode *MD =
 cast (this)->getMetadata(LLVMContext::MD_fpmath);
 if (!MD)
 return 0.0;
 ConstantFP *Accuracy = mdconst::extract (MD->getOperand(0));
...
unsigned CastInst::isEliminableCastPair(Instruction::CastOps firstOp,
 ...
 if ((IsFirstBitcast && isa (SrcTy) != isa (MidTy)) ||
 (IsSecondBitcast && isa (MidTy) != isa (DstTy)))
 if (!AreBothBitcasts)
 return 0;
...
bool CastInst::isBitCastable(Type *SrcTy, Type *DestTy) {
 ...
 if (PointerType *DestPtrTy = dyn_cast (DestTy)) {
 if (PointerType *SrcPtrTy = dyn_cast (SrcTy)) {
...
bool CastInst::isBitOrNoopPointerCastable(Type *SrcTy, Type *DestTy,
 ...
 if (auto *PtrTy = dyn_cast (SrcTy))
 if (auto *IntTy = dyn_cast (DestTy))
 return (IntTy->getBitWidth() == DL.getPointerTypeSizeInBits(PtrTy) &&
 !DL.isNonIntegralPointerType(PtrTy));
 if (auto *PtrTy = dyn_cast (DestTy))
 if (auto *IntTy = dyn_cast (SrcTy))
 return (IntTy->getBitWidth() == DL.getPointerTypeSizeInBits(PtrTy) &&
 !DL.isNonIntegralPointerType(PtrTy));
...
CastInst::castIsValid(Instruction::CastOps op, Type *SrcTy, Type *DstTy) {
 ...
 case Instruction::BitCast: {
 PointerType *SrcPtrTy = dyn_cast (SrcTy->getScalarType());
 PointerType *DstPtrTy = dyn_cast (DstTy->getScalarType());
...
void CmpInst::swapOperands() {
 if (ICmpInst *IC = dyn_cast (this))
 IC->swapOperands();
 else
 cast (this)->swapOperands();
}

bool CmpInst::isCommutative() const {
 if (const ICmpInst *IC = dyn_cast (this))
 return IC->isCommutative();
 return cast (this)->isCommutative();
}
...
static bool hasNonZeroFPOperands(const CmpInst *Cmp) {
 auto *LHS = dyn_cast (Cmp->getOperand(0));
 auto *RHS = dyn_cast (Cmp->getOperand(1));
...
std::optional ICmpInst::compare(const KnownBits &LHS,
 const KnownBits &RHS,
 ICmpInst::Predicate Pred) {
 switch (Pred) {
...
static bool isImpliedTrueByMatchingCmp(CmpPredicate Pred1, CmpPredicate Pred2) {
 ...
 case CmpInst::ICMP_ULT: // A s B implies A != B and A >=s B are true.
 return Pred2 == CmpInst::ICMP_NE || Pred2 == CmpInst::ICMP_SGE;
 case CmpInst::ICMP_SLT: // A  ICmpInst::isImpliedByMatchingCmp(CmpPredicate Pred1,
 CmpPredicate Pred2) {
...
std::optional CmpPredicate::getMatching(CmpPredicate A,
 CmpPredicate B) {
 if (A.Pred == B.Pred)
...
void SwitchInstProfUpdateWrapper::addCase(
 ...
 if (!Weights && W && *W) {
 Changed = true;
 Weights = SmallVector (SI.getNumSuccessors(), 0);
...
SwitchInstProfUpdateWrapper::getSuccessorWeight(const SwitchInst &SI,
 ...
 return mdconst::extract (ProfileData->getOperand(idx + 1))
 ->getValue()
 .getZExtValue();
...
bool UnreachableInst::shouldLowerToTrap(bool TrapUnreachable,
 ...
 if (const CallInst *Call = dyn_cast_or_null (getPrevNode());
