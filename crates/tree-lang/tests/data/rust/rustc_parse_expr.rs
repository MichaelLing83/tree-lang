...
impl<'a> Parser<'a> {
 ...
 #[inline]
 pub fn parse_expr(&mut self) -> PResult<'a, Box > {
 ...
 pub fn parse_expr_force_collect(&mut self) -> PResult<'a, Box > {
 self.current_closure.take();
 ...
 fn parse_expr_paren_seq(&mut self) -> PResult<'a, ThinVec >> {
 ...
 fn parse_expr_prefix_range(&mut self, attrs: AttrWrapper) -> PResult<'a, Box > {
 if !attrs.is_empty() {
 ...
 fn parse_expr_prefix(&mut self, attrs: AttrWrapper) -> PResult<'a, Box > {
 ...
 fn parse_expr_prefix_common(&mut self, lo: Span) -> PResult<'a, (Span, Box)> {
 self.bump();
 let attrs = self.parse_outer_attributes()?;
 ...
 lhs_span: Span,
 op_span: Span,
 expr_kind: fn(Box, Box) -> ExprKind,
 ) -> PResult<'a, Box > {
 let mk_expr = |this: &mut Self, lhs: Box, rhs: Box | {
 ...
 *self = parser_snapshot_after_type;
 ...
 fn parse_expr_dot_or_call(&mut self, attrs: AttrWrapper) -> PResult<'a, Box > {
 self.collect_tokens_for_expr(attrs, |this, attrs| {
 ...
 pub(crate) fn parse_floating_field_access(&mut self) -> PResult<'a, Vec > {
 ...
 fn parse_expr_fn_call(&mut self, lo: Span, fun: Box) -> Box {
 ...
 fn parse_expr_index(&mut self, lo: Span, base: Box) -> PResult<'a, Box > {
 ...
 fn parse_dot_suffix(&mut self, self_arg: Box, lo: Span) -> PResult<'a, Box > {
 ...
 fn parse_expr_bottom(&mut self) -> PResult<'a, Box > {
 ...
 fn parse_expr_lit(&mut self) -> PResult<'a, Box > {
 let lo = self.token.span;
 ...
 fn parse_expr_tuple_parens(&mut self, restrictions: Restrictions) -> PResult<'a, Box > {
 let lo = self.token.span;
 ...
 fn parse_expr_path_start(&mut self) -> PResult<'a, Box > {
 ...
 fn recover_do_catch(&mut self) -> PResult<'a, Box > {
 let lo = self.token.span;
 ...
 fn parse_expr_opt(&mut self) -> PResult<'a, Option >> {
 Ok(if self.token.can_begin_expr() { Some(self.parse_expr()?) } else { None })
 }

 /// Parse `"return" expr?`.
 fn parse_expr_return(&mut self) -> PResult<'a, Box > {
 ...
 /// Parse `"do" "yeet" expr?`.
 fn parse_expr_yeet(&mut self) -> PResult<'a, Box > {
 let lo = self.token.span;
 ...
 fn parse_expr_become(&mut self) -> PResult<'a, Box > {
 let lo = self.prev_token.span;
 ...
 fn parse_expr_break(&mut self) -> PResult<'a, Box > {
 let lo = self.prev_token.span;
 ...
 *expr = next;
 ...
 fn parse_expr_continue(&mut self, lo: Span) -> PResult<'a, Box > {
 let mut label = self.eat_label();
 ...
 /// Parse `"yield" expr?`.
 fn parse_expr_yield(&mut self) -> PResult<'a, Box > {
 let lo = self.prev_token.span;
 ...
 fn parse_expr_builtin(&mut self) -> PResult<'a, Box > {
 ...
 &mut self,
 parse: impl FnOnce(&mut Parser<'a>, Span, Ident) -> PResult<'a, Option >,
 ) -> PResult<'a, T> {
 let lo = self.token.span;
 ...
 pub(crate) fn parse_expr_offset_of(&mut self, lo: Span) -> PResult<'a, Box > {
 ...
 pub(crate) fn parse_expr_type_ascribe(&mut self, lo: Span) -> PResult<'a, Box > {
 let expr = self.parse_expr()?;
 self.expect(exp!(Comma))?;
 ...
 pub fn parse_str_lit(&mut self) -> Result > {
 ...
 pub fn eat_token_lit(&mut self) -> Option {
 let check_expr = |expr: Box | {
 ...
 /// Matches `lit = true | false | token_lit`.
 /// Returns `None` if the next token is not a literal.
 fn parse_opt_meta_item_lit(&mut self) -> Option {
 self.recover_after_dot();
 let span = self.token.span;
 ...
 pub fn parse_literal_maybe_minus(&mut self) -> PResult<'a, Box > {
 if let Some(expr) = self.eat_metavar_seq_with_matcher(
 |mv_kind| matches!(mv_kind, MetaVarKind::Expr { .. }),
 |this| {
 ...
 /// expression.
 fn maybe_suggest_brackets_instead_of_braces(&mut self, lo: Span) -> Option > {
 ...
 fn parse_simple_block(&mut self) -> PResult<'a, Box > {
 let blk = self.parse_block()?;
 Ok(self.mk_expr(blk.span, ExprKind::Block(blk, None)))
 }

 /// Parses a closure expression (e.g., `move |args| expr`).
 fn parse_expr_closure(&mut self) -> PResult<'a, Box > {
 let lo = self.token.span;
 ...
 fn parse_closure_block_body(&mut self, ret_span: Span) -> PResult<'a, Box > {
 if self.may_recover()
 ...
 fn parse_fn_block_decl(&mut self) -> PResult<'a, (Box, Span)> {
 ...
 fn parse_expr_if(&mut self) -> PResult<'a, Box > {
 let lo = self.prev_token.span;
 ...
 fn parse_if_after_cond(&mut self, lo: Span, mut cond: Box) -> PResult<'a, Box > {
 ...
 fn parse_expr_let(&mut self, restrictions: Restrictions) -> PResult<'a, Box > {
 ...
 fn parse_expr_else(&mut self) -> PResult<'a, Box > {
 ...
 fn error_on_extra_if(&mut self, cond: &Box) -> PResult<'a, ()> {
 ...
 pub fn parse_for_head(&mut self) -> PResult<'a, (Pat, Box)> {
 ...
 /// Parses `for await? in ` (`for` token already eaten).
 fn parse_expr_for(&mut self, opt_label: Option, lo: Span) -> PResult<'a, Box > {
 let is_await =
 ...
 fn parse_expr_while(&mut self, opt_label: Option, lo: Span) -> PResult<'a, Box > {
 ...
 fn parse_expr_loop(&mut self, opt_label: Option, lo: Span) -> PResult<'a, Box > {
 ...
 fn parse_expr_match(&mut self) -> PResult<'a, Box > {
 let match_span = self.prev_token.span;
 let attrs = self.parse_outer_attributes()?;
 ...
 let err = |this: &Parser<'_>, stmts: Vec | {
 ...
 pub(crate) fn eat_metavar_guard(&mut self) -> Option > {
 self.eat_metavar_seq(MetaVarKind::Guard, |this| this.parse_match_arm_guard()).flatten()
 }

 fn parse_match_arm_guard(&mut self) -> PResult<'a, Option >> {
 ...
 fn parse_try_block(&mut self, span_lo: Span) -> PResult<'a, Box > {
 let annotation =
 ...
 *t == token::OpenBrace
 ...
 fn parse_gen_block(&mut self) -> PResult<'a, Box > {
 let lo = self.token.span;
 ...
 *t == token::OpenBrace || t.is_metavar_block()
 ...
 ThinVec,
 ast::StructRest,
 Option, /* async blocks are forbidden in Rust 2015 */
 ),
 > {
 ...
 fn mk_assign_op(&self, assign_op: AssignOp, lhs: Box, rhs: Box) -> ExprKind {
 ...
 fn mk_unary(&self, unop: UnOp, expr: Box) -> ExprKind {
 ExprKind::Unary(unop, expr)
 }

 fn mk_binary(&self, binop: BinOp, lhs: Box, rhs: Box) -> ExprKind {
 ExprKind::Binary(binop, lhs, rhs)
 }

 fn mk_index(&self, expr: Box, idx: Box, brackets_span: Span) -> ExprKind {
 ExprKind::Index(expr, idx, brackets_span)
 }

 fn mk_call(&self, f: Box, args: ThinVec >) -> ExprKind {
 ExprKind::Call(f, args)
 }

 fn mk_await_expr(&mut self, self_arg: Box, lo: Span) -> Box {
 let span = lo.to(self.prev_token.span);
 ...
 fn mk_use_expr(&mut self, self_arg: Box, lo: Span) -> Box {
 let span = lo.to(self.prev_token.span);
 ...
 pub(crate) fn mk_expr(&self, span: Span, kind: ExprKind) -> Box {
 self.mk_expr_with_attrs(span, kind, AttrVec::new())
 }

 pub(super) fn mk_expr_err(&self, span: Span, guar: ErrorGuaranteed) -> Box {
 self.mk_expr(span, ExprKind::Err(guar))
 }

 pub(crate) fn mk_unit_expr(&self, span: Span) -> Box {
 self.mk_expr(span, ExprKind::Tup(Default::default()))
 }

 pub(crate) fn mk_closure_expr(&self, span: Span, body: Box) -> Box {
 self.mk_expr(
 span,
 ...
 fn mk_expr_sp(&self, lhs: &Box, lhs_span: Span, op_span: Span, rhs_span: Span) -> Span {
 lhs.attrs
 .iter()
 ...
 &mut self,
 attrs: AttrWrapper,
 f: impl FnOnce(&mut Self, ast::AttrVec) -> PResult<'a, Box >,
 ) -> PResult<'a, Box > {
 self.collect_tokens(None, attrs, ForceCollect::No, |this, attrs| {
